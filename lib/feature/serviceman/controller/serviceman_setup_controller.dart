...
class ServicemanSetupController extends GetxController  with GetSingleTickerProviderStateMixin implements GetxService{
  ...
  Future<void> getSingleServicemanData({int? index,String? fromPage}) async {
    if(fromPage=='editPage'){
      ...
      countryDialCode = VerifyPhoneHelper.getValidCountryCode(servicemanList?[index!].phone ?? "") != "" ? VerifyPhoneHelper.getValidCountryCode(servicemanList?[index!].phone ?? "") : CountryCode.fromCountryCode(Get.find<SplashController>().configModel.content!.countryCode!).dialCode??"+880";
      firstNameController.text = servicemanList?[index!].firstName ?? "";
      lastNameController.text =  servicemanList?[index!].lastName ?? "";
      phoneController.text = VerifyPhoneHelper.getValidPhone(servicemanList?[index!].phone ?? "") != "" ? VerifyPhoneHelper.getValidPhone(servicemanList?[index!].phone ?? "") : servicemanList?[index!].phone ?? "";
      ...
      countryDialCode = VerifyPhoneHelper.getValidCountryCode(Get.find<ServicemanDetailsController>().servicemanModel?.user?.phone ?? "") != "" ? VerifyPhoneHelper.getValidCountryCode(Get.find<ServicemanDetailsController>().servicemanModel?.user?.phone ?? "") : CountryCode.fromCountryCode(Get.find<SplashController>().configModel.content!.countryCode!).dialCode??"+880";
      ...
      phoneController.text = VerifyPhoneHelper.getValidPhone(Get.find<ServicemanDetailsController>().servicemanModel?.user?.phone ?? "") != "" ? VerifyPhoneHelper.getValidPhone(Get.find<ServicemanDetailsController>().servicemanModel?.user?.phone ?? "") : servicemanList?[index!].phone ?? "";
  ...
  Future<void> changeServicemanStatus(int index,String servicemanId,{bool fromDetailsPage = false}) async {
   ...
   if(response.statusCode==200 && response.body['response_code']=='default_200'){
     ...
     showCustomSnackBar("serviceman_status_updated_successfully".tr, isError: false);
   }else{
     showCustomSnackBar("${response.body['message']}");
   }
      update();
